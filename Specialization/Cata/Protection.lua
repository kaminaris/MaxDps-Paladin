...
function Protection:precombat()
    ...
    if (MaxDps:CheckSpellUsable(classtable.SealofTruth, 'SealofTruth')) and (buff[classtable.SealBuff].remains <360 or not buff[classtable.SealBuff].up) and cooldown[classtable.SealofTruth].ready and not UnitAffectingCombat('player') then
        if not setSpell then setSpell = classtable.SealofTruth end
    end
    if (MaxDps:CheckSpellUsable(classtable.DivinePlea, 'DivinePlea')) and (( HolyPower <3 or mana.percent <false ) and false) and cooldown[classtable.DivinePlea].ready and not UnitAffectingCombat('player') then
...
function Protection:cleave()
    if (MaxDps:CheckSpellUsable(classtable.LayOnHands, 'LayOnHands')) and (curentHP <false) and cooldown[classtable.LayOnHands].ready then
        if not setSpell then setSpell = classtable.LayOnHands end
    end
    if (MaxDps:CheckSpellUsable(classtable.GuardianofAncientKings, 'GuardianofAncientKings')) and (curentHP <false and not buff[classtable.ArdentDefenderBuff].up) and cooldown[classtable.GuardianofAncientKings].ready then
        if not setSpell then setSpell = classtable.GuardianofAncientKings end
    end
    if (MaxDps:CheckSpellUsable(classtable.ArdentDefender, 'ArdentDefender')) and (curentHP <false and not buff[classtable.GuardianofAncientKingsBuff].up) and cooldown[classtable.ArdentDefender].ready then
        if not setSpell then setSpell = classtable.ArdentDefender end
    end
    if (MaxDps:CheckSpellUsable(classtable.HolyShield, 'HolyShield')) and (curentHP <false and not buff[classtable.DefensiveBuff].up) and cooldown[classtable.HolyShield].ready then
        if not setSpell then setSpell = classtable.HolyShield end
    end
    if (MaxDps:CheckSpellUsable(classtable.DivineProtection, 'DivineProtection')) and (curentHP <false and not buff[classtable.DefensiveBuff].up) and cooldown[classtable.DivineProtection].ready then
        if not setSpell then setSpell = classtable.DivineProtection end
    end
    if (MaxDps:CheckSpellUsable(classtable.DivineGuardian, 'DivineGuardian')) and (curentHP <false and not buff[classtable.DefensiveBuff].up and false) and cooldown[classtable.DivineGuardian].ready then
        if not setSpell then setSpell = classtable.DivineGuardian end
    end
    if (MaxDps:CheckSpellUsable(classtable.LayOnHands, 'LayOnHands')) and (curentHP <false) and cooldown[classtable.LayOnHands].ready then
        if not setSpell then setSpell = classtable.LayOnHands end
    end
    if (MaxDps:CheckSpellUsable(classtable.SealofTruth, 'SealofTruth')) and (not buff[classtable.SealBuff].up) and cooldown[classtable.SealofTruth].ready then
        if not setSpell then setSpell = classtable.SealofTruth end
    end
    if (MaxDps:CheckSpellUsable(classtable.AvengingWrath, 'AvengingWrath')) and cooldown[classtable.AvengingWrath].ready then
        if not setSpell then setSpell = classtable.AvengingWrath end
    end
    if (MaxDps:CheckSpellUsable(classtable.DivinePlea, 'DivinePlea')) and (HolyPower == 0 or mana.percent <false) and cooldown[classtable.DivinePlea].ready then
        if not setSpell then setSpell = classtable.DivinePlea end
    end
    if (MaxDps:CheckSpellUsable(classtable.WordofGlory, 'WordofGlory')) and (HolyPower == 3 and curentHP <false) and cooldown[classtable.WordofGlory].ready then
...
function Protection:single()
    ...
    if (MaxDps:CheckSpellUsable(classtable.HammerofWrath, 'HammerofWrath')) and (targetHP <20) and cooldown[classtable.HammerofWrath].ready then
